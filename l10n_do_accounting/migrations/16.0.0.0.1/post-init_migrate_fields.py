...
def migrate_ref_field(env):
    ...
    query = """
    UPDATE account_move SET l10n_do_fiscal_number = ref,
     l10n_do_sequence_prefix=SUBSTRING(ref FROM 1 FOR 3),
      l10n_do_sequence_number = CASE 
            WHEN regexp_replace(SUBSTRING(ref FROM 4 FOR 13), '\D', '', 'g') <> '' 
                THEN CAST(regexp_replace(SUBSTRING(ref FROM 4 FOR 13), '\D', '', 'g') AS INTEGER) 
            ELSE NULL 
        END 
